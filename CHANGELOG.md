...
- **站点导航：** 新增固定顶栏（总览 / 规则 / 处罚）与左侧文档栏（按分组列出全部文档、当前页高亮；窄屏收起到屏外，由顶栏「目录」按钮滑出），首页新增头图与六张套色文档卡片，页内目录改为可展开的折叠块
- **视觉对齐官网：** 站点采用官网（www.aqcraft.cn）Forty 模板的色板与字形——底色 `#242943`、面板 `#2a2f4a`、强调色 `#9bf1ff`、1px 半透明分隔线 `rgba(212,212,255,.1)`、Source Sans Pro 字栈、纯直角、描边按钮；不再提供亮/暗切换（官网为单一暗色）
